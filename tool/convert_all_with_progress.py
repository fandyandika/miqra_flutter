#!/usr/bin/env python3
"""Convert all tajwid with visible progress output."""
...
def main():
    ...
    # Print to both stderr (visible) and log file
    def log(msg):
        print(msg, file=sys.stderr)
        with open(log_file, 'a', encoding='utf-8') as f:
            f.write(msg + '\n')
    
    log(f"=== Starting conversion at {datetime.now()} ===")
    
    # Parse XML
    log("Step 1/5: Parsing Uthmani XML...")
        ...
        for aya in sura.findall('aya'):
            ...
            if sn not in (1, 9) and an == 1:
                ...
                if b:
                    txt = b + ' ' + txt
            ud[sn][an] = txt
    log(f"  ✓ Loaded {len(ud)} surahs")
    
    # Parse SQL
    log("Step 2/5: Parsing Indopak SQL...")
    id_ = {}
    count = 0
    with open(str(sp), 'r', encoding='utf-8') as f:
        for line in f:
            ...
            try:
                ...
                continue
    log(f"  ✓ Loaded {len(id_)} surahs, {count} verses")
    
    # Load tajwid
    log("Step 3/5: Loading Uthmani tajwid data...")
    with open(str(tp), 'r', encoding='utf-8') as f:
        tj = json.load(f)
    log(f"  ✓ Loaded {len(tj)} tajwid entries")
    
    # Convert
    log("Step 4/5: Converting tajwid data (this may take 2-3 minutes)...")
    surahs = {}
    tc = 0
    ts = 0
    last_log = 0
    ...
    for idx, e in enumerate(tj):
        ...
        if (idx + 1) - last_log >= 500:
            pct = ((idx + 1) / len(tj)) * 100
            log(f"  Progress: {idx + 1}/{len(tj)} ({pct:.1f}%) - {tc} converted, {ts} skipped")
            last_log = idx + 1
    
    log(f"  ✓ Conversion complete: {tc} converted, {ts} skipped")
    
    # Save
    log("Step 5/5: Saving JSON files...")
    import os
    ...
    for sn in sorted(surahs.keys()):
        ...
        if sn <= 5 or sn % 20 == 0:
            ...
            log(f"  ✓ {fn}: {len(sd['verses'])} verses, {tsp} spans")
    
    log(f"\n=== Conversion complete at {datetime.now()} ===")
    log(f"Generated {len(surahs)} surah files")
    log(f"Total: {tc} spans converted, {ts} spans skipped")
    log("\nNext: Run 'dart run bin/validate_tajwid.dart' to validate")

#!/usr/bin/env python3
"""
Generate Accurate Indopak Tajwid using Needleman-Wunsch Alignment

This script directly generates accurate tajwid JSON files for Indopak
by aligning Uthmani and Indopak texts and converting indices.
"""
...
def score_char(char1: str, char2: str) -> int:
    """Score for character matching."""
    if char1 == char2:
        return 2
    # Base character match (ignore diacritics)
    code1, code2 = ord(char1), ord(char2)
    if 0x0600 <= code1 <= 0x06FF and 0x0600 <= code2 <= 0x06FF:
        # Get base character (not combining marks 064B-065F)
        base1 = char1 if (code1 < 0x064B or code1 > 0x065F) else ''
        base2 = char2 if (code2 < 0x064B or code2 > 0x065F) else ''
...
def main():
    ...
    print("=" * 60)
    print("Generate Accurate Indopak Tajwid")
    print("=" * 60)
    print()
    ...
    for entry in uthmani_tajwid:
        ...
        if len(surahs) <= 3 and ayah_num <= 3:
            print(f"  Surah {surah_num} Ayah {ayah_num}: {len(filtered_spans)} spans")

...
# Data yang diharapkan dari user (array format)
expected_data = [
...
print(f"Loaded {len(surahs)} surahs from JSON")
print(f"Expected {len(expected_data)} surahs from reference data")
print("\nMembandingkan name_id dan name_id_translation...\n")
print(f"{'No':<4} {'Field':<25} {'Expected':<50} {'Actual':<50}")
print("=" * 135)

for idx, expected_item in enumerate(expected_data, start=1):
    surah = surahs[idx - 1]  # Array index starts at 0
    num = surah['surah']
    exp_name = expected_item['name_id']
    exp_trans = expected_item['name_id_translation']
    ...
    if exp_name != act_name:
        ...
        print(f"{num:<4} {'name_id':<25} {exp_name:<50} {act_name:<50}")
    ...
    if exp_trans != act_trans:
        ...
        print(f"{num:<4} {'name_id_translation':<25} {exp_trans:<50} {act_trans:<50}")
...
# Write to file
output_path = os.path.join(script_dir, 'surah_differences.txt')
with open(output_path, 'w', encoding='utf-8') as f:
    f.write(f"Total perbedaan: {len(differences)}\n\n")
    f.write(f"{'No':<4} {'Field':<25} {'Expected':<50} {'Actual':<50}\n")
    f.write("=" * 135 + "\n")
    for diff in differences:
        f.write(f"{diff['num']:<4} {diff['field']:<25} {diff['expected']:<50} {diff['actual']:<50}\n")
print(f"\n✅ Hasil juga ditulis ke {output_path}")

#!/usr/bin/env python3
"""
Generate all accurate Indopak tajwid JSON files using difflib alignment.

This script processes all surahs/ayahs and generates accurate tajwid data.
"""
...
def main():
    ...
    log_file = project_root / "tajwid_generation.log"
    
    with open(log_file, 'w', encoding='utf-8') as log:
        log.write("=" * 60 + "\n")
        log.write("Generate Accurate Indopak Tajwid\n")
        log.write("=" * 60 + "\n\n")
        
        # Parse
        log.write("Parsing texts...\n")
        log.flush()
        uthmani_data = parse_uthmani_xml(str(xml_path))
        indopak_data = parse_indopak_sql(str(sql_path))
        log.write(f"Uthmani: {len(uthmani_data)} surahs\n")
        log.write(f"Indopak: {len(indopak_data)} surahs\n\n")
        
        # Load tajwid
        log.write("Loading Uthmani tajwid...\n")
        log.flush()
        with open(uthmani_tajwid_path, 'r', encoding='utf-8') as f:
            uthmani_tajwid = json.load(f)
        log.write(f"Loaded {len(uthmani_tajwid)} entries\n\n")
        ...
        log.write("Converting tajwid data...\n")
        log.flush()
        ...
        for idx, entry in enumerate(uthmani_tajwid):
            ...
            if (idx + 1) % 1000 == 0:
                log.write(f"  Processed {idx + 1}/{len(uthmani_tajwid)} entries...\n")
                log.flush()
        ...
        for surah_num in sorted(surahs.keys()):
            ...
            if surah_num <= 5:
                log.write(f"  {filename}: {len(surah_data['verses'])} verses, {total_spans} spans\n")
        
        log.write(f"\n✓ Generated {len(surahs)} surah files\n")
        log.write(f"✓ Converted {total_converted} spans\n")
        if total_skipped > 0:
            log.write(f"⚠ Skipped {total_skipped} spans\n")
        
        log.write("\n" + "=" * 60 + "\n")
        log.write("Complete!\n")
        log.write("=" * 60 + "\n")
    
    print(f"Generation complete! Check {log_file} for details.")
